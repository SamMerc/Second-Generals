...
#Path to store model
model_save_path = base_dir+'Model_Storage/nn_only/'
check_and_make_dir(model_save_path)
#Path to store plots
plot_save_path = base_dir+'Plots/nn_only/'
...
class CustomDataModule(pl.LightningDataModule):
    def __init__(self, train_inputs, train_outputs, valid_inputs, valid_outputs, test_inputs, test_outputs, batch_size, rng):
        ...
        # --- Input scaling ---
        # This is the "scaling layer at the start of the network": the NN has no
        # ens-CGP step to standardize its inputs, so a StandardScaler here feeds
        # directly into the model's input_proj (D -> hidden_dim) layer, which is
        # the learned dimensionality-expansion step analogous to what ens-CGP did.
        in_scaler_phys = StandardScaler()
        in_scaler_phys.fit(train_inputs.cpu().numpy())

        def scale_inputs(X):
            return torch.tensor(in_scaler_phys.transform(X.cpu().numpy()), dtype=torch.float32)
...
# --- Inputs: physical parameters only. Unlike TP_GP_MLP.py, there are no ---
# --- ens-CGP outputs/errors to append since the NN is doing the full     ---
# --- D -> 2*O mapping by itself.                                        ---
...
# --- Outputs: with no ens-CGP baseline to correct, the NN targets the raw ---
# --- T-P profile directly instead of a residual.                         ---
...
#Comparing NN predicted T-P profiles vs true T-P profiles with residuals
substep = 100
...
NN_res_T = np.zeros(NN_test_outputs_T.shape, dtype=float)
NN_res_P = np.zeros(NN_test_outputs_P.shape, dtype=float)

for NN_test_idx, (NN_test_input, true_T, true_P) in enumerate(zip(
    NN_test_inputs, NN_test_outputs_T, NN_test_outputs_P
)):
    ...
    #Inverse scaling - NN predicts the profile directly (no GP baseline to add)
    NN_pred_output_T = out_scaler_T.inverse_transform(NN_pred_output[:, :O].reshape(1, -1)).flatten()
    NN_pred_output_P = out_scaler_P.inverse_transform(NN_pred_output[:, O:].reshape(1, -1)).flatten()
...
ax1.plot(NN_res_T.T, alpha=0.1, color='blue')
ax2.plot(NN_res_P.T, alpha=0.1, color='blue')
for ax in [ax1, ax2]:
